...
while event_queue.len() > 0:
    ...
    adapt_start = time()
    anomaly = event_queue.head().anomaly
    total_cs_in_system -= event_queue.head().cybersickness
    ...
    wait_times.append((anomaly, chosen_solution, elapsed_time, event_queue.head().wait_time, overall_response_time, time() + overall_response_time - start, event_queue.head().entry_time, time(), event_queue.head().cybersickness, total_cs_in_system))
    event_queue.pop()
    # event_queue.resort()
...
write('end_to_end_log_BHPQ.csv', wait_times, 'w')
# write('end_to_end_log_noresort.csv', wait_times, 'w')
